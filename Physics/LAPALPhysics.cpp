...
void LAPAL::updateLinearVelocity(LAPAL::movementData& mData, const float dTime, float& objMass) {
...
bool LAPAL::checkTerrainCollision(const LAPAL::plane3f& terrain,const LAPAL::vec3f& position){
    ...
    if (equ == 0){
        collision=true;
        std::cout << "entrando en" << position.x << position.y << position.z << collision << std::endl;
    }
